...
public class Osap extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (ParamCalculate.compareTo("true") == 0 || ParamAjax.compareTo("true") == 0) { // call from UI.jspx
			...
			if (loan.m_hasError == false) {
				...
				loan.m_strMessage = "";
				String pa= request.getPathInfo(); 
				if (ParamAjax.compareTo("true") == 0)
				{
					//response.setContentType("application/json");
					PrintWriter out = response.getWriter();
					out.printf("Grace Period Interest: $%.2f<br/>Monthly payment: $%.2f",
							loan.m_graceInterest, loan.m_monthlyPayment);
					out.flush();
					return;
				}
		...
		if (loan.m_principal > loan.m_maxPrincipal) {
			loan.m_maxPrincipal = loan.m_principal;
			session.setAttribute("ParamMaxPrincipal", loan.m_maxPrincipal);
		}
